export class InfiniteBackgroundScene extends Phaser.Scene {
  ...
  this.player.play("fly", true);
    ...
    updateTimer() {
      ...
      // Si estamos en la mitad del tiempo, creamos obstáculos lentos una sola vez
      if (this.timeLimit === this.midTime && !this.slowObstaclesCreated) {
        this.spawnSlowObstacle();
      }
  
      // Si el tiempo se acaba, determinamos victoria o derrota y reiniciamos
      if (this.timeLimit <= 0) {
